...
#
class Game:
    MISSILE_SHAPE = [(-10, 0), (0, +3), (10, 0), (0, -3)]
    ...
    def __init__(self):
        ...
        self.space.collision_handler(
            self.PLAYER_COL_TYPE, 4, post_solve=self.on_missile_collision
        )
        self.space.collision_handler(
            4, self.BASE_COL_TYPE, post_solve=self.on_missile_floor_base_collision
        )
        self.space.collision_handler(
            4, self.FLOOR_COL_TYPE, post_solve=self.on_missile_floor_base_collision
        )
    ...
    def on_missile_collision(self, arbiter: Arbiter):
        self.landed = False
        self.victory = False
        for _ in range(100):
            self.particles.emmit(
                position = arbiter.contact_point_set.points[0].point_b,
                velocity = Vec2d(random.uniform(15,30), 0).rotated(random.uniform(-150, 160))
            )
        self.space.remove(self.player.shape, self.player)
        self.landed = True
        return True

    def on_missile_floor_base_collision(self, arbiter:Arbiter):
        if len(self.missiles) > 0 and arbiter.is_first_contact:
            self.space.remove(arbiter.shapes[0])
        return True
    ...
    def spawn_missiles(self):
        ...
        dx = random.uniform(-WIDTH, WIDTH)
        for _ in range(1):
            missile = self.space.create_poly(self.MISSILE_SHAPE,mass=1,moment=2,position=self.player.position + (dx, HEIGHT),friction=1.0,collision_type= 4,filter=ShapeFilter(group=2), color=pyxel.COLOR_YELLOW)

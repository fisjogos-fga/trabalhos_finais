...
class Particlesflare:
    ...
    def emmit_end(self, position, velocity):
        ...
        p.duration = 85 - random.expovariate(1 / 10)
        p.velocity_func = self.update_velocity
        self.particles.append(p)
...
class Game:
    ...
    def drop_bomb(self):
        position = self.player.position - (0, 4)
        vel = self.player.rotation_vector.rotated(-90) * 120
        bomb = self.space.create_circle(
            2, position=position, color=pyxel.COLOR_ORANGE, velocity=vel
        )
        bomb.life = 90
    ...
    def update(self):
        ...
        if not self.landed:
            ...
            if pyxel.btn(pyxel.KEY_UP):
                ...
                for _ in range(3):
                    self.particles.emmit(
                        ...
                        velocity=-random.uniform(70, 100)
                        * self.player.rotation_vector.perpendicular(),
                    )
            ...
            if pyxel.btnp(pyxel.KEY_B):
                self.player.apply_force_at_local_point(100 * self.THRUST)
            ...
            if (
                200 < self.player.velocity.y < 202
                or 200 < self.player.velocity.x < 202
                or -200 < self.player.velocity.y < -202
                or -200 < self.player.velocity.x < -202
            ):
                for _ in range(100):
                    self.particlesbarrer.emmit(
                        ...
                        velocity=60
                        * self.player.rotation_vector.perpendicular().rotated(
            ...
            if (
                700 < self.player.velocity.y < 702
                or 700 < self.player.velocity.x < 702
                or -700 < self.player.velocity.y < -702
                or -700 < self.player.velocity.x < -702
            ):
                for _ in range(100):
                    self.particlesbarrer.emmit(
            ...
            if pyxel.btnp(pyxel.KEY_SPACE):
                for _ in range(1):
                    self.particlesflare.emmit_flare(
                        position=self.player.local_to_world((random.uniform(0, 0), -3)),
                        velocity=-random.uniform(-700, -700)
                        * self.player.rotation_vector.perpendicular(),
                    )
                ...
                for _ in range(2):
                    for _ in range(80):
                        self.particlesexplosion.emmit(
                            ...
                            velocity=-random.uniform(-150, -150)
                            * self.player.rotation_vector.perpendicular().rotated(
                                random.uniform(0, 360)
                            ),
                        )
    ...
    def update_bombs(self):
        for bomb in self.bombs:
            bomb.life -= 1
            if bomb.life <= 60 and bomb.life % 5 == 0:
                bomb.color = pyxel.COLOR_WHITE
            elif bomb.life % 4 == 0:
                bomb.color = pyxel.COLOR_ORANGE
            if bomb.life == 0:
                for _ in range(100):
                    self.particlesflare.emmit_end(
                        position=bomb.position + (0, 0),
                        velocity=50
                        * self.player.rotation_vector.perpendicular().rotated(
                            random.uniform(0, 360)
                        ),
